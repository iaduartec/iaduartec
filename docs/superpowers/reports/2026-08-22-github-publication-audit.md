...
| root/InsForge | README and idempotent ownership migration now enforce owner-scoped RLS, enabled RLS, and least-privilege API grants; no runtime data tracked. | Local `insforge` runtime verified `owner_id`, policies, RLS enabled, no `anon` grants, and anonymous denial. Cloud CLI remains unavailable because its stored API key is invalid. | Published with the root documentation/migration commit; cloud re-apply requires a valid credential. |
