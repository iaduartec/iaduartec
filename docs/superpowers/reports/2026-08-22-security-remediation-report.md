...
- InsForge: no `USING (true)`/`WITH CHECK (true)` remains in trading migrations. The local `insforge` database now has `owner_id`, owner-scoped policies, enabled RLS, `(owner_id, asset_symbol)` uniqueness, and no `anon` table grants; anonymous access is denied and an authenticated role without a JWT owner sees zero rows. The CLI cloud context remains unusable because its stored API key is invalid, so this evidence is explicitly local-runtime scope and cloud deployment still needs a valid credential.
